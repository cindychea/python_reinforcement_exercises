...
// (i.e. by adding an additional person property in each step object), and each committee member has an equal number of tasks. 
...
counter = 0
function assignMember(committee, steps) {
    while (counter < project.steps.length) {
        project.committee.forEach(member => {
...
assignMember(project.committee, project.steps)
